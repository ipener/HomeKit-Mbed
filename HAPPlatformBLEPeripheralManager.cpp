...
class _USBSerial: public USBSerial {
protected:
...
static uint8_t                   _chrLastAddedIndex = 0;
static uint8_t                   _chrIndex = 0;
...
HAPError HAPPlatformBLEPeripheralManagerAddService(
    ...
    GattService svc { { type->bytes, UUID::LSB }, &_chrs[_chrLastAddedIndex], (uint16_t)(_chrIndex - _chrLastAddedIndex) };
    ...
    for (auto i = _chrLastAddedIndex; i < _chrIndex; ++i) {
...
HAPError HAPPlatformBLEPeripheralManagerAddCharacteristic(
    ...
    if (constNumBytes && constBytes) {
        _chrs[_chrIndex] = new GattCharacteristic { { type->bytes, UUID::LSB }, (uint8_t*)constBytes, (uint16_t)constNumBytes, (uint16_t)constNumBytes, prop, nullptr, 0};
    } else {
        _chrs[_chrIndex] = new GattCharacteristic { { type->bytes, UUID::LSB }, nullptr, 0, ATT_VALUE_MAX_LEN, prop, &_dscs[_chrIndex], 1};
        _chrs[_chrIndex]->setReadAuthorizationCallback(handleReadRequest);
        _chrs[_chrIndex]->setWriteAuthorizationCallback(handleWriteRequest);
    }
    _handles[_chrIndex].value = valueHandle;
    _handles[_chrIndex].cccd = cccDescriptorHandle;
    _chrIndex++;
...
HAPError HAPPlatformBLEPeripheralManagerAddDescriptor(
    ...
    _dscs[_chrIndex] = new GattAttribute { { type->bytes, UUID::LSB }, (uint8_t*)constBytes, (uint16_t)constNumBytes, (uint16_t)constNumBytes };
    _handles[_chrIndex].iid = descriptorHandle;
...
void HAPPlatformBLEPeripheralManagerStartAdvertising(
    ...
    if (auto err = gap.setAdvertisingParameters(_advertisingHandle, adv_params)) {
...
void HAPPlatformBLEPeripheralManagerStopAdvertising(HAPPlatformBLEPeripheralManagerRef blePeripheralManager) {
    ...
    if (!gap.isAdvertisingActive(_advertisingHandle)) return;
